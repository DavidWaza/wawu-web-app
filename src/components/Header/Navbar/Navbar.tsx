...
const Navbar: React.FC<linkProps> = () => {
  ...
  return (
    <>
      <nav className="bg-white w-2/3 top-[4rem] left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-[50px] fixed rounded-full flex items-center px-10 z-10">
        <div className="flex justify-between items-center w-full">
          <div className="flex">
            <Logo />
            <div className="hidden gap-4 pt-[2.7rem] z-10 text-black 2xl:flex">
              {links.map((link) => (
                <ul key={link.id}>
                  <Link href={link.href}>
